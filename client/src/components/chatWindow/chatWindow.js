...
export default class ChatWindow extends Component {
    ...
    async componentDidMount() {
        ...
        try {
            ...
            let contactsResult = await API.getContacts(this.props.loggedInUserObj.user._id, this.props.loggedInUserObj.user.role)
            
            this.setState({ users: contactsResult.data })
            console.log(this.props.signalProtocolManagerUser);
        ...
        ws.onmessage = async (e) => {
            ...
            if (newMessage.senderid === this.props.loggedInUserObj.user._id) {
                newMessage.message = this.state.lastSentMessage
                console.log("chat window testing 56");
                newMessage.message="hi darshan";
                ...
                console.log("chat window testing 62");
                newMessage.message="hi darshan";
            ...
            if (newMessage.chatId in this.state.chats) {
                ...
                }), () => localStorage.setItem(this.props.loggedInUserObj.user._id + "_messages", JSON.stringify(this.state.chats)))
            }
    ...
    async getNewMsgObj(newMsgObj) {
        ...
        let msgToSend = { chatId: selectedUserChatId, senderid: this.props.loggedInUserObj.user._id, receiverid: this.state.messageToUser._id, ...newMsgObj }
